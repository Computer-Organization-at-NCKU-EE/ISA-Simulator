...
static uop_t Core_decode (Core *self, inst_t inst) {
    ...
    case BRANCH: {
        ...
        ret.reg_rs1_val   = self->arch_state.gpr[rs1];
        ret.reg_rs2_val   = self->arch_state.gpr[rs2];
        ret.imm_signext = (inst.B_TYPE.imm_12 << 12) + (inst.B_TYPE.imm_11 << 11) +
    ...
    case JAL: {
        ret.reg_rd_index = rd;
        ret.imm_signext = (inst.J_TYPE.imm_20 << 20) + (inst.J_TYPE.imm_19_12 << 12) +
...
static void Core_execute (Core *self, uop_t uop) {
    ...
    if (uop.is_mem) {
        ...
        if (uop.is_store) {
            ...
            MemoryMap_generic_store (&self->mem_map, alu_cal_result, len, _ref_data);
        } else {
            byte_t _buffer[4] = {};
            MemoryMap_generic_load (&self->mem_map, alu_cal_result, len, _buffer);
